...
import com.example.forecast.components.ForecastDetailDailyTile
import com.example.forecast.components.ForecastDetailMoreInfo
import com.example.forecast.components.ForecastDetailScreenWeatherCard
import com.example.forecast.components.ForecastDetailsScreenTopBar
...
fun DetailScreenPortrait(
    ...
    Column(
        ...
        LazyColumn {
            ...
            if (!state.forecast?.list.isNullOrEmpty()) {
                items(count = state.forecast?.list!!.size) {
                    ForecastDetailDailyTile(state.forecast?.list!![it])
                }
            }
            if (state.currentWeather != null) {
                item {
...
fun DetailScreenLandscape(
    ...
    Row {
        ...
        LazyColumn(
            ...
            if (state.isLoading && state.forecast?.list.isNullOrEmpty()) {
                item { UiLoader() }
            }
            if (!state.forecast?.list.isNullOrEmpty()) {
                items(count = state.forecast?.list!!.size) {
